...
import kafka.utils.{Json, ZkUtils, CoreUtils}
...
class ZkRackLocator(zkClient: ZkClient, props: Properties) extends RackLocator(zkClient, props) {
  ...
  def getBrokerRack(brokerId: Int): String = {
    val infoString = zkUtils.readDataMaybeNull(ZkUtils.BrokerIdsPath + "/" + brokerId)._1
  ...
  override def getRackInfo(): Map[Int, String] = {
    val brokers = zkUtils.getSortedBrokerList()
...
object RackLocator {
  ...
  def create(zkClient: ZkClient, locatorClass: String = "", props: String = ""): RackLocator = {
    if (locatorClass == "" || locatorClass == "kafka.cluster.NoRack") {
