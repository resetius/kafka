...
object KafkaConfig {
  ...
  val ReplicaFetchAvgBytesProp = "replica.fetch.avg.bytes"
  ...
  val ReplicaFetchAvgBytesDoc = "Average bytes expected for each fetch response."
  ...
  private val configDef = {
    ...
    new ConfigDef()
      ...
      .define(ReplicaFetchAvgBytesProp, INT, Defaults.ReplicaFetchMaxBytes, HIGH, ReplicaFetchAvgBytesDoc)
      ...
      .define(SaveUncleanEnableProp, BOOLEAN, Defaults.SaveUncleanEnable, HIGH, SaveUncleanEnableDoc)
...
class KafkaConfig(val props: java.util.Map[_, _], doLog: Boolean) extends AbstractConfig(KafkaConfig.configDef, props, doLog) {
  ...
  val replicaFetchAvgBytes = getInt(KafkaConfig.ReplicaFetchAvgBytesProp)
  ...
  val saveUncleanEnable: java.lang.Boolean = getBoolean(KafkaConfig.SaveUncleanEnableProp)
  ...
  private def validateValues() {
    ...
    require(replicaFetchAvgBytes <= replicaFetchMaxBytes)
