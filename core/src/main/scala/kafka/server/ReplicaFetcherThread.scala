...
class ReplicaFetcherThread(name: String,
  ...
  def handleOffsetOutOfRange(topicAndPartition: TopicAndPartition): Long = {
    ...
    if (leaderEndOffset < replica.logEndOffset.messageOffset) {
      ...
      val config = LogConfig.fromProps(brokerConfig.originals, AdminUtils.fetchEntityConfig(replicaMgr.zkUtils,
        ConfigType.Topic, topicAndPartition.topic))
      if (!config.uncleanLeaderElectionEnable && !config.saveUncleanEnable) {
        // Log a fatal error and shutdown the broker to ensure that data loss does not unexpectedly occur.
        fatal("Halting because log truncation is not allowed for partition %s,".format(topicAndPartition) +
      ...
      replicaMgr.logManager.truncateTo(Map(topicAndPartition -> leaderEndOffset), config.saveUncleanEnable)
      leaderEndOffset
    } else {
      /**
