...
class ZkUtils(val zkClient: ZkClient, 
   ...
   * @param jmxPort
   */
  def registerBrokerInZk(id: Int, rackId: String, host: String, port: Int, advertisedEndpoints: immutable.Map[SecurityProtocol, EndPoint], jmxPort: Int) {
    val brokerIdPath = BrokerIdsPath + "/" + id
    val timestamp = SystemTime.milliseconds.toString

    val brokerInfo = Json.encode(Map("version" -> 2, "host" -> host, "rackId" -> rackId,  "port" -> port, "endpoints"->advertisedEndpoints.values.map(_.connectionString).toArray, "jmx_port" -> jmxPort, "timestamp" -> timestamp))
   ...
   */
  private object ConditionalUpdatePersistentPathCallback extends StatCallback {
    ...
    def processResult(rc: Int, path: String, internalCtx: scala.Any, stat: Stat): Unit = {
      ...
      Code.get(rc) match {
        ...
        case Code.CONNECTIONLOSS | Code.SESSIONEXPIRED =>
          ...
          zkHandle.setData(path, ZKStringSerializer.serialize(data), expectVersion, ConditionalUpdatePersistentPathCallback, internalCtx)
        case _ =>
  ...
  def conditionalUpdatePersistentPathAsync(path: String, data: String, expectVersion: Int,
    ...
    this.zkConnection.getZookeeper.setData(path, ZKStringSerializer.serialize(data), expectVersion, ConditionalUpdatePersistentPathCallback, internalCtx)
  }
   ...
   */
  private object ReadDataMaybeNullCallback extends DataCallback {
  ...
  def readDataMaybeNullAsync(path: String, callback: ((Option[String], Stat), scala.Any) => Unit, ctx: scala.Any): Unit = {
    ...
    this.zkConnection.getZookeeper.getData(path, false, ReadDataMaybeNullCallback, internalCtx)
  }
